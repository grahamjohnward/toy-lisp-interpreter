...
(defun macroexpand-all (form)
  ...
  (let ((sym (car form)))
    (cond ((eq sym 'if) (macroexpand-all-if form))
	  ((eq sym 'tagbody) (macroexpand-all-tagbody form))
	  ((eq sym 'progn) `(progn ,@(macroexpand-all-list (cdr form))))
	  ((eq sym 'condition-case) (macroexpand-all-condition-case form))
	  ((eq sym 'let) (let ((clauses (second form))
			       (body (cdr (cdr form))))
			   `(let ,(macroexpand-all-let clauses)
			      ,@(macroexpand-all-list body))))
	  ((eq sym 'quote) form)
	  ((eq sym 'quasiquote) `(,sym ,@(macroexpand-all-quasiquote (cdr form) 1)))
	  ((eq sym 'function) (macroexpand-all-function form))
