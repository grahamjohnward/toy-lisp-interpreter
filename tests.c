...
static void test_vm_push_pop()
{
    ...
    lisp_object_t code_vector = parse1_wrapper("#(0 foo 0 bar)");
...
static void test_vm_call_builtin()
{
    ...
    lisp_object_t code_vector = parse1_wrapper("#(0 foo 0 bar 0 2 0 cons 1)");
    vm_set_code_vector(&interp->vm, code_vector);
    vm_run(&interp->vm);
...
static void test_vm_inst_call_lambda()
{
    ...
    /* Set up the stack */
    vm_inst_push(&interp->vm, 0, sym("foo"));
    vm_inst_push(&interp->vm, 0, sym("bar"));
    vm_inst_push(&interp->vm, 0, LispInt(2));
    vm_inst_push(&interp->vm, 0, symbol);
    /* Call */
    vm_inst_call(&interp->vm, 0);
    vm_run(&interp->vm);
    /* Check */
...
static void test_vm_inst_get_set()
{
    ...
    vm_inst_get(&interp->vm, 0, 0, LispInt(1));
    check(vm_pop(&interp->vm) == sym("foo"), "get");
    vm_inst_get(&interp->vm, 0, LispInt(1), LispInt(2));
    check(vm_pop(&interp->vm) == sym("xyz"), "get closure env");

    vm_inst_push(&interp->vm, 0, sym("baz"));
    vm_inst_set(&interp->vm, 0, 0, LispInt(1));
    vm_inst_get(&interp->vm, 0, 0, LispInt(1));
    check(vm_pop(&interp->vm) == sym("baz"), "set");

    vm_inst_push(&interp->vm, 0, sym("boo"));
    vm_inst_set(&interp->vm, 0, LispInt(1), LispInt(2));
    vm_inst_get(&interp->vm, 0, LispInt(1), LispInt(2));
...
static void test_vm_function()
{
    ...
    lisp_object_t lambda_code = parse1_wrapper("#(14 1 0 hello 2 0 1 0 2 0 cons 1 8)");
    ...
    lisp_object_t calling_code = parse1_wrapper("#(0 world 0 1 0 placeholder 1)");
...
static void test_vm_function_return_from()
{
    ...
    lisp_object_t lambda_code = parse1_wrapper("#(14 1 11 g19 25 0 g19 2 0 1 0 :bog 0 1 0 symbol-value 1 0 2 0 cons 1 0 2 16 4 8)");
    lisp_object_t fn = allocate_function();
    struct lisp_function *fnptr = LispFunctionPtr(fn);
    fnptr->kind = interp->syms.lambda;
    lisp_object_t arg_info = parse1_wrapper("#(nil 1)");
    ...
    /* Some code to call the function */
    lisp_object_t calling_code = parse1_wrapper("#(0 boo 0 1 0 foo 1)");
...
static void test_vm_inst_jmp()
{
    ...
    lisp_object_t code = parse1_wrapper("#(9 4 0 foo 0 bar)");
...
static void test_vm_inst_jmp_if_nil1()
{
    ...
    lisp_object_t code = parse1_wrapper("#(0 nil 5 6 0 foo 0 bar)");
    vm_set_code_vector(&interp->vm, code);
    vm_run(&interp->vm);
...
static void test_vm_inst_jmp_if_nil2()
{
    ...
    lisp_object_t code = parse1_wrapper("#(0 abc 5 6 0 foo 0 bar)");
    vm_set_code_vector(&interp->vm, code);
    vm_run(&interp->vm);
...
static void test_vm_inst_set_tag()
{
    ...
    vm_inst_set_tag(&interp->vm, 0, sym("bof"), LispInt(4));
...
static void test_vm_inst_tag_jmp()
{
    ...
    vm_inst_set_tag(vm, 0, sym("bof"), LispInt(4));
...
static void test_vm_inst_raise()
{
    ...
    *vm->call_stack_pointer = vm->registers;
    ...
    vm_inst_push(vm, 0, sym("bof")); /* tag */
    vm_inst_push(vm, 0, sym("return-value")); /* return value */
    vm_inst_push(vm, 0, LispInt(2)); /* argcount */
...
void test_vm_inst_get0()
{
    ...
    vm_inst_push(vm, 0, LispInt(1));
    vm_inst_push(vm, 0, LispInt(2));
    vm_inst_push(vm, 0, LispInt(2)); // arg count
    vm_inst_setup_env2(vm, 0, LispInt(2));

    vm_inst_get0(vm, 0, LispInt(1));
    check(vm->top_of_data_stack - vm->data_stack == 1, "size1");
    check(vm_pop(vm) == LispInt(1), "ok1");

    vm_inst_get0(vm, 0, LispInt(2));
...
void test_vm_inst_set0()
{
    ...
    vm_inst_push(vm, 0, LispInt(14));
    vm_inst_set0(vm, 0, LispInt(1));
...
void test_vm_function_stack_allocated()
{
    ...
    lisp_object_t lambda_code = parse1_wrapper("#(10 2 0 hello 6 1 0 2 0 cons 1 6 2 0 2 0 cons 1 8)");
    /* Make the above code the function value of a symbol */
    lisp_object_t fn = allocate_function();
    struct lisp_function *fnptr = LispFunctionPtr(fn);
    fnptr->kind = interp->syms.lambda;
    ...
    /* Some code to call the function */
    lisp_object_t calling_code = parse1_wrapper("#(0 beautiful 0 world 0 2 0 foo 1)");
